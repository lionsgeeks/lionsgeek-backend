<div class="hidden md:block">
    <div :class="darkmode ? 'bg-black text-white' : 'bg-white'"
        ...
        <div class="flex flex-col w-full">
            ...
            <div id="nav-content" class='flex flex-col gap-[0.5rem] py-7 px-[0.5rem] relative'>
                <x-nav-item route="dashboard" icon="dashboard" label="Dashboard" :active="request()->routeIs('dashboard')" />
                <x-nav-item route="events.index" icon="events" label="Events" :active="request()->routeIs('events.index') ||
                    request()->routeIs('events.create') ||
                    request()->routeIs('events.show')" />

                <div class="flex flex-col items-start gap-2 w-[100%] ">
                    <!-- Dropdown Toggle Button -->
                    <div :class="darkmode ? 'bg-black text-white hover:bg-gray-800 hover:text-alpha' : 'bg-white hover:bg-gray-300'"
                        id="dropdownButton1"
                        class="w-[100%] cursor-pointer nav-button no-underline  text-base font-bold flex gap-2 items-center   py-[0.75rem] px-[1rem]  rounded-xl transition-all duration-300">
                        <button class="flex items-center gap-2 w-[100%] ">
                            <x-nav-icon name="articles" />
                    ...
                    <!-- Dropdown Menu -->
                    <div id="dropdownMenu1" :class="darkmode ? 'bg-black text-white' : 'bg-white'"
                        class="hidden  flex-col gap-2 group-hover:ms-5 ms-0 w-[91%]   rounded-lg shadow-md transition-all duration-300">

                        <x-nav-item route="blogs.index" icon="blogs" label="Blogs" :active="request()->routeIs('blogs.index') ||
                            request()->routeIs('blogs.create') ||
                            request()->routeIs('blogs.show')" />

                        <x-nav-item route="press.index" icon="press" label="Press" :active="request()->routeIs('press.index') ||
                            request()->routeIs('press.create') ||
                            request()->routeIs('press.show')" />

                    </div>
                </div>


                <x-nav-item route="projects.index" icon="projects" label="Projects" :active="request()->routeIs('projects.index') ||
                ...
                <div class="flex flex-col items-start gap-2 w-[100%] ">
                    ...
                    <div :class="darkmode ? 'bg-black text-white hover:bg-gray-800 hover:text-alpha' : 'bg-white hover:bg-gray-300'"
                        id="dropdownButton2"
                        class="w-[100%] nav-button no-underline cursor-pointer text-base font-bold flex gap-2 items-center   py-[0.75rem] px-[1rem]   rounded-xl  transition-all duration-300">
                        <button class="flex items-center gap-2 w-[100%] ">
                            <x-nav-icon name="connect" />
                            <div class="hidden group-hover:flex truncate w-[100%] justify-between items-center gap-2">
                                Connect
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                    ...
                    <!-- Dropdown Menu -->
                    <div id="dropdownMenu2" :class="darkmode ? 'bg-black text-white' : 'bg-white'"
                        class="hidden flex-col gap-2 group-hover:ms-5 ms-0 w-[91%]   rounded-lg shadow-md transition-all duration-300">

                        <x-nav-item route="contacts.index" icon="contacts" label="Contacts" :active="request()->routeIs('contacts.index') ||
                            request()->routeIs('contacts.create') ||
                            request()->routeIs('contacts.show')" />
                        <x-nav-item route="coworkings.index" icon="coworking" label="Coworking" :active="request()->routeIs('coworkings.index') ||
                            request()->routeIs('coworkings.create') ||
                            request()->routeIs('coworkings.show')" />
                    </div>
                </div>
                <x-nav-item route="gallery.index" icon="gallery" label="Gallery" :active="request()->routeIs('gallery.index') ||
                    request()->routeIs('gallery.create') ||
                    request()->routeIs('gallery.show')" />
                <x-nav-item route="newsletter.index" icon="newsletter" label="newsletter" :active="request()->routeIs('newsletter.index') ||
                    request()->routeIs('newsletter.create') ||
                    request()->routeIs('newsletter.show')" />
                <div class="flex flex-col items-start gap-2 w-[100%] ">
                    <!-- Dropdown Toggle Button -->
                    <div :class="darkmode ? 'bg-black text-white hover:bg-gray-800 hover:text-alpha' : 'bg-white hover:bg-gray-300'"
                        id="dropdownButton3"
                        class="w-[100%] nav-button no-underline text-base font-bold flex gap-2 items-center py-[0.75rem] px-[1rem]   rounded-xl  cursor-pointer transition-all duration-300">
                        <button class="flex items-center gap-2 w-[100%] ">
                            <x-nav-icon name="sp" />
                            <div class="hidden group-hover:flex truncate w-[100%] justify-between items-center gap-2">
                    ...
                    <!-- Dropdown Menu -->
                    <div id="dropdownMenu3" :class="darkmode ? 'bg-black text-white' : 'bg-white'"
                        class="hidden flex-col gap-2 group-hover:ms-5 ms-0 w-[91%]   rounded-lg shadow-md transition-all duration-300">
                        <x-nav-item route="infosessions.index" icon="session" label="Info Sessions" :active="request()->routeIs('infosessions.index') ||
                            request()->routeIs('infosessions.create') ||
                            request()->routeIs('infosessions.show')" />

                        <x-nav-item route="participants.index" icon="participants" label="Participants"
                            :active="request()->routeIs('participants.index') ||
                                request()->routeIs('participants.create') ||
                                request()->routeIs('participants.show')" />
                    </div>
                </div>
            </div>
        </div>
...
<div class="md:hidden z-50">
    <div
        ...
        <div class="flex flex-col items-start gap-2 w-[100%] ">
            <!-- Dropdown Toggle Button -->
            <div
                class="w-[100%] nav-button no-underline text-base font-bold flex gap-2 items-center   py-[0.75rem] px-[1rem] bg-black text-white  rounded-xl  hover:text-alpha hover:bg-gray-800 transition-all duration-300">
            ...
            <div id="dropdownMenu4"
                class="hidden  flex-col gap-2 ms-5 w-[91%]  bg-black text-white rounded-lg shadow-md transition-all duration-300">
        ...
        <a href='{{ route('gallery.index') }}'
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="currentColor" class="bi bi-card-image"
                viewBox="0 0 16 16">
        ...
        <a href='{{ route('newsletter.index') }}'
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="currentColor" class="bi bi-envelope"
                viewBox="0 0 16 16">
                <path
